...
```console
$ pytest tests/calculations/test_phonopy.py
```
...
```console
$ uv run pytest tests/calculations/test_phonopy.py
```
...
```console
$ hatch test tests/calculations/test_phonopy.py
```
...
In the following, we assume that the latest release was `v1.8.0`.
Furthermore, we assume the following naming conventions for remotes:

* `origin`: remote pointing to `aiida-phonopy/aiida-phonopy`
* `fork`: remote pointing to personal fork, e.g. `bastonero/aiida-phonopy`
...
* Patch release: `v1.8.0` to `v1.8.1`, only bug fixes
* Minor release: `v1.8.0` to `v1.9.0`, bug fixes and new features that **maintain** backwards compatibility
* Major release: `v1.8.0` to `v2.0.0`, bug fixes and new features that **break** backwards compatibility
...
For most releases, we assume that all the changes in the current `main` branch have to be included in the release. As such, branch off the new release branch directly from `main`:
    ...
    git fetch --all --prune
    git checkout origin/main -b release/1.9.0
...
* Also check the [compatibility matrix in `README.md`](https://github.com/aiida-phonopy/aiida-phonopy?tab=readme-ov-file#compatibility) to see if any changes have to be made.

Once you've prepared the release branch locally, commit with the message 'Release `v1.9.0`' and push it to Github. For our major/minor release example:

    git commit -am 'Release `v1.9.0`'
    git push origin release/1.9.0
...
Make sure the remote `release/1.9.0` branch is up to date by pushing the local changes, then go to Github and create a pull request to the `main` branch of the official repository.

After the pull request has been approved, merge the PR using the "Squash and Merge", and make sure the commit is simply named e.g. 'Release `v1.9.0`'.
...
:::{important}
In case you did push with the wrong tag, the CD should fail since we [validate the tag](https://github.com/aiida-phonopy/aiida-phonopy/blob/main/.github/workflows/validate_release_tag.py) as part of the workflow.
