...
from aiida_phonopy.common.utils import (
    phonopy_atoms_to_structure, phonopy_atoms_from_structure)
from aiida_phonopy.workflows.iter_ha import (
    _extract_dataset_from_db, _create_dataset)
load_profile()
...
def bunch_phonons(pks, pk_nac, max_items=None, include_ratio=None,
                  linear_decay=True):
    ...
    d, f, e, idx = _create_dataset(
        displacements, forces, energies, max_items, include_ratio,
...
def bunch_band_phonopy(pks, pk_nac, max_items=None, include_ratio=None,
                       linear_decay=True):
    ph = bunch_phonons(pks, pk_nac, max_items=max_items,
                       include_ratio=include_ratio,
                       linear_decay=linear_decay)
    print("max_items: %d, include_ratio: %f, linear_decay: %s, snapshots: %d"
          % (max_items, include_ratio, linear_decay,
             ph.dataset['displacements'].shape[0]))
...
def get_bunch_tp_phonopy(pks, pk_nac, temperature, max_items=None,
                         include_ratio=None, linear_decay=True):
    ph = bunch_phonons(pks, pk_nac, max_items=max_items,
                       include_ratio=include_ratio,
                       linear_decay=linear_decay)
    ph.produce_force_constants(fc_calculator='alm')
...
def bunch_dump_phonopy(pks, pk_nac, max_items=None, include_ratio=None,
                       linear_decay=True):
    ph = bunch_phonons(pks, pk_nac, max_items=max_items,
                       include_ratio=include_ratio,
                       linear_decay=linear_decay)
...
def main(launch, pk, uuid, pks, bunch_dump, bunch_band, bunch_free_energy,
         list_pks, show_uuid, pk_nac, num_prev, num_tail,
         include_ratio, linear_decay):
    """
    ...
    if launch:
        ...
        if show_uuid:  # Show latest IterHarmonicApprox node uuid
            ...
            print("All PhonopyWorkChain nodes: ",
                  ",".join(["%d" % pk for pk in search_pk(_uuid)]))
            if len(_pks) > 0:
                print("PhonopyWorkChain nodes used to generate random "
                      "displacements at each iteration:")
                pks_list = get_pks_list(_pks, get_num_prev(_uuid),
            ...
            for pk_set in get_pks_list(_pks, _num_prev,
                                       initial_pks=initial_nodes,
                                       num_tail=num_tail):
                bunch_dump_phonopy(pk_set, _pk_nac, max_items=_num_prev,
